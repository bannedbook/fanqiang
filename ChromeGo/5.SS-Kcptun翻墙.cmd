...
echo 等待SS-Kcptun启动，请稍候...
IF EXIST %~dp0Browser\chrome.exe (
    start %~dp0Browser\chrome.exe --user-data-dir=%~dp0chrome-user-data --proxy-server=127.0.0.1:1080 https://www.bannedbook.org/bnews/fq/?utm_source=SS-Kcptun
) ELSE (
	%SystemRoot%\System32\reg.exe query "HKLM\Software\Microsoft\Windows\CurrentVersion\App Paths\chrome.exe" >nul 2>&1
	IF  not errorlevel 1 (
		start chrome.exe --user-data-dir=%~dp0chrome-user-data --proxy-server=127.0.0.1:1080 https://www.bannedbook.org/bnews/fq/?utm_source=SS-Kcptun
	) else ( 
